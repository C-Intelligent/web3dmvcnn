...
from models.custom_vgg import Custom_VGG
...
class SVCNN(Model):
    ...
    def __init__(self, name, nclasses=55, pretraining=True, cnn_name='vgg11'):
...
class MVCNN(Model):
    ...
    def __init__(self, name, model, nclasses=55, cnn_name='vgg11', num_views=12, pretrained=True):
        super(MVCNN, self).__init__(name)

        self.nclasses = nclasses
        self.pretraining = pretrained
        ...
        if self.use_resnet:
            if self.cnn_name == 'resnet18':
                rn = models.resnet18(pretrained=self.pretraining)
                self.net_1 = nn.Sequential(
                    rn.conv1, rn.layer1, rn.layer2, rn.layer3, rn.layer4
                )
                self.avgpool = rn.avgpool
                self.net_2 = nn.Linear(512,self.nclasses)
            elif self.cnn_name == 'resnet34':
                rn = models.resnet34(pretrained=self.pretraining)
                self.net_1 = nn.Sequential(
                    rn.conv1, rn.bn1, rn.relu, rn.maxpool, rn.layer1, rn.layer2, rn.layer3, rn.layer4
                )
                self.avgpool = rn.avgpool
                self.net_2 = nn.Linear(512,self.nclasses)
            elif self.cnn_name == 'resnet50':
                rn = models.resnet50(pretrained=self.pretraining)
                self.net_1 = nn.Sequential(
                    rn.conv1, rn.bn1, rn.relu, rn.maxpool, rn.layer1, rn.layer2, rn.layer3, rn.layer4
                )
                self.avgpool = rn.avgpool
                self.net_2 = nn.Linear(2048,self.nclasses)
        else:
            if self.cnn_name == 'alexnet':
                self.net_1 = models.alexnet(pretrained=self.pretraining).features
                self.net_2 = models.alexnet(pretrained=self.pretraining).classifier
    ...
    def forward(self, x):
        ...
        if self.use_resnet:
            y = self.avgpool(torch.max(y,1)[0])
